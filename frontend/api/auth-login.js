...
const { Pool } = pg;
const pool = globalThis.__nroraAuthLoginPool || new Pool({ connectionString: process.env.DATABASE_URL, max: 5, idleTimeoutMillis: 10000 });
globalThis.__nroraAuthLoginPool = pool;
...
export default async function handler(req,res){
  ...
  }catch(e){console.error("NRORA login error",e);return json(res,500,{error:e?.message||"Login server error"});}
}
