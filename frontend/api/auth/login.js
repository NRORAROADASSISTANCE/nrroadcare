...
async function ensureUser() {
  ...
  if (!ceo.rowCount) await pool.query("insert into users(username,password_hash,role,name) values($1,$2,'ceo',$3)",[CEO_USERNAME, hash(CEO_PASSWORD), "NRORA CEO"]);
  else await pool.query("update users set role='ceo', name='NRORA CEO', password_hash=$1, active=true where username=$2",[hash(CEO_PASSWORD), CEO_USERNAME]);
...
export default async function handler(req, res) {
  ...
  try {
    await ensureUser();
    const { username, password } = req.body || {}; const loginUsername = String(username || "").trim().replace(/^@+/, "");
    if (!loginUsername || typeof password !== "string" || !password) return res.status(400).json({ error: "Username and password are required" });
    const result = await pool.query("select * from users where lower(username)=lower($1) and active=true",[loginUsername]);
    ...
    const mech = await pool.query("select id,name,phone,username,password_hash,active from technicians where lower(username)=lower($1) and active=true",[loginUsername]);
    const m = mech.rows[0];
