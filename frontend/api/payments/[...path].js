...
const SECRET=cleanEnv(process.env.SESSION_SECRET)||"change-this-session-secret-in-vercel";
...
async function razor(path,method="GET",body){
  if(!RAZORPAY_KEY_ID||!RAZORPAY_KEY_SECRET) throw new Error("Razorpay credentials are missing in Vercel Production.");
  ...
  if(!r.ok){
    ...
    if(r.status===401){
      const secretFp=crypto.createHash("sha256").update(RAZORPAY_KEY_SECRET).digest("hex").slice(0,12);
      throw new Error(`Razorpay rejected Production credentials (HTTP 401). Key=${RAZORPAY_KEY_ID.slice(0,18)}… Secret fingerprint=${secretFp}.${requestId?` Request ID=${requestId}`:""}`);
    }
...
async function capture(orderId,paymentId,amount,method){
  const q=await pool.query("select * from payments where gateway_order_id=$1 limit 1",[orderId]); if(!q.rowCount)return null;
  const p=q.rows[0]; if(Number(amount)!==Number(p.amount))return null;
...
async function captureQr(paymentRow,qrPayment){
  if(!qrPayment||qrPayment.status!=="captured")return null; const amount=Number(qrPayment.amount)/100; if(amount!==Number(paymentRow.amount))return null;
...
export default async function handler(req,res){
  ...
  try{
    const u=requireAuth(req,res); if(!u)return; await schema();
    ...
    if(req.method==="POST"&&path==="create-order"){
      ...
      let qr=null,qrError="";
      try{qr=await razor("/payments/qr_codes","POST",{type:"upi_qr",name:`NRORA-${customerId}`,usage:"single_use",fixed_amount:true,payment_amount:450000,description:"NRORA Yearly Road Assistance Membership",notes:{customer_id:String(customerId),payment_id:String(p.rows[0].id),vehicle_no:String(customer.vehicle_no||"")}}); await pool.query("update payments set qr_code_id=$1 where id=$2",[qr.id,p.rows[0].id]);}catch(e){qrError=e?.message||"QR generation failed";}
    ...
    if(req.method==="POST"&&path==="verify"){
      const {razorpay_order_id,razorpay_payment_id,razorpay_signature}=req.body||{}; if(!razorpay_order_id||!razorpay_payment_id||!razorpay_signature)return json(res,400,{error:"Incomplete payment verification data"});
      const expected=crypto.createHmac("sha256",RAZORPAY_KEY_SECRET).update(`${razorpay_order_id}|${razorpay_payment_id}`).digest("hex"); if(expected!==razorpay_signature)return json(res,400,{error:"Payment signature verification failed"});
      const p=await razor(`/payments/${encodeURIComponent(razorpay_payment_id)}`); if(p.status!=="captured")return json(res,409,{error:`Payment is ${p.status}. Receipt will be created only after capture.`});
      const done=await capture(razorpay_order_id,razorpay_payment_id,Number(p.amount)/100,p.method); if(!done)return json(res,409,{error:"Payment could not be confirmed"}); return json(res,200,done);
    }
    if(req.method==="GET"&&path==="status"){
      const orderId=String(req.query?.order_id||""); if(!orderId)return json(res,400,{error:"order_id is required"});
      const r=await pool.query("select p.*,c.name customer_name,c.phone,c.vehicle_no,r.receipt_no,r.created_at receipt_created_at from payments p left join customers c on c.id=p.customer_id left join receipts r on r.payment_id=p.id where p.gateway_order_id=$1 limit 1",[orderId]); if(!r.rowCount)return json(res,404,{error:"Payment not found"});
      const row=r.rows[0];
      if(row.status!=="captured"&&row.qr_code_id){try{const qrPayments=await razor(`/payments/qr_codes/${encodeURIComponent(row.qr_code_id)}/payments?count=10`); const qp=(qrPayments.items||[]).find(x=>x.status==="captured"&&Number(x.amount)===Number(row.amount)*100); if(qp){const done=await captureQr(row,qp); if(done)return json(res,200,{...done.payment,customer_name:row.customer_name,phone:row.phone,vehicle_no:row.vehicle_no,receipt_no:done.receipt.receipt_no,receipt_created_at:done.receipt.created_at});}}catch(e){console.error("QR status check failed",e);}}
      const latest=await pool.query("select p.*,c.name customer_name,c.phone,c.vehicle_no,r.receipt_no,r.created_at receipt_created_at from payments p left join customers c on c.id=p.customer_id left join receipts r on r.payment_id=p.id where p.id=$1 limit 1",[row.id]); return json(res,200,latest.rows[0]);
    }
